...
# Use built in class from pymunk to create Static floor
static_body = pm.Body(body_type=pm.Body.STATIC)
static_lines = [pm.Segment(static_body, (0.0, 060.0), (1200.0, 060.0), 0.0)] #no radius for segment
static_lines1 = [pm.Segment(static_body, (1200.0, 060.0), (1200.0, 800.0), 0.0)]
#Static lines are for floor of our game
...
def sling_action():
    ...
    uv = unit_vector(v) #Set unit vector
    uv1 = uv[0]
    uv2 = uv[1]
    mouse_distance = distance(sling_x, sling_y, x_mouse, y_mouse) #mouse distance
    pu = (uv1*rope_length+sling_x, uv2*rope_length+sling_y) #present the displacement of sling
    bigger_rope = 102
    x_redbird = x_mouse - 20
    y_redbird = y_mouse - 20
    #If the sling stretch
...
load_music()
#Level from level.py
level = Level(pigs, columns, beams, space) #create object level
level.number = 0
level.load_level() #call function

#Main Game Loop
while running:
    # Input handling
    for event in pygame.event.get():
        #when the user click X button at the window -> Quit
        if event.type == pygame.QUIT:
            running = False
        # Or Press ESC to exit the game
        elif event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
            running = False
        #Press K to active or deactive wall
